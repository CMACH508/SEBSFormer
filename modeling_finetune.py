...
from module_GDGF import GDGF
from module_MSTAM import MS_TAM
...
class FrequencyLayer(nn.Module):
    def __init__(self, attn_drop, embed_dim, time, c):
        super(FrequencyLayer, self).__init__()
        self.out_dropout = nn.Dropout(attn_drop)
        self.LayerNorm = nn.LayerNorm(embed_dim)
        self.c = c // 2 + 1
        self.time = time
        self.sqrt_beta = nn.Parameter(torch.randn(1, 1, embed_dim))

    def forward(self, input_tensor):
        batch, seq_len, hidden = input_tensor.shape
        dim4_tensor = input_tensor[:, 1:, :].reshape(batch, -1, self.time, hidden)
        x = torch.fft.rfft(dim4_tensor, dim=2, norm='ortho')
        low_pass = x[:]
        low_pass[:, :, self.c:, :] = 0
        low_pass = torch.fft.irfft(low_pass, n=self.time, dim=2, norm='ortho')
        high_pass = dim4_tensor - low_pass
        sequence_emb_fft = low_pass + (self.sqrt_beta**2) * high_pass

        hidden_states = self.out_dropout(sequence_emb_fft)
        hidden_states = self.LayerNorm(hidden_states + dim4_tensor) 
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
        ...
        self.filters = nn.ModuleList([
            FrequencyLayer(
                attn_drop=attn_drop_rate, embed_dim=embed_dim, time=4, c=2)     #Workload
            for i in range(depth)])
        ...
        self.gdgf = GDGF(feature_dim=embed_dim, num_regions=4)
        self.mstam = MS_TAM(T=4, r=2)                                         #Workload
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        x = self.pos_drop(x)
        x_filtlist = []
        for index, (fil, blk) in enumerate(zip(self.filters, self.blocks)):
            x_filt_tmp = fil(x)                        
            x_filt_tmp = x_filt_tmp.permute(0, 2, 1, 3)
            x_filt_tmp, _  = self.mstam(x_filt_tmp)       
            x_filt_tmp, _ = self.gdgf(x_filt_tmp) 
            x_filtlist.append(x_filt_tmp)
            x = blk(x, rel_pos_bias=None) 
        x_filt = torch.cat(x_filtlist, dim=1)     
        x_filt = self.attention(x_filt)           
        x_filt = self.norm(x_filt)               
        x = self.norm(x)                  
        ...
        if self.fc_norm is not None:
            ...
            if return_patch_tokens:
                ...
                t = t.mean(1)            
                t = self.fc_norm(t)      
                t_filt = x_filt.mean(1)         
                t_filt = self.fc_norm(t_filt)       
                return torch.cat((t, t_filt), dim=1)
        else:
            if return_all_tokens:
                return x
