...
def _to_device_dense(x, device):
    if torch.is_tensor(x):
        x = x.to(device, non_blocking=True)
        return x.to_dense() if x.is_sparse else x
    if isinstance(x, (list, tuple)):
        return type(x)(_to_device_dense(t, device) for t in x)
    if isinstance(x, dict):
        return {k: _to_device_dense(v, device) for k, v in x.items()}
    return x  # 例如 ch_names 是字符串列表，原样返回即可
