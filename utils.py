...
class WorkloadLoader(torch.utils.data.Dataset):
    def __init__(self, root, files, sampling_rate=200):
        self.root = root
        self.files = files
        self.default_rate = 200
...
def prepare_Workload_dataset(root):
    # set random seed
    seed = 12345
    np.random.seed(seed)
    ...
    # prepare training and test data loader
    train_dataset = WorkloadLoader(os.path.join(root, "train"), train_files)
    test_dataset = WorkloadLoader(os.path.join(root, "test"), test_files)
    val_dataset = WorkloadLoader(os.path.join(root, "val"), val_files)
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset
